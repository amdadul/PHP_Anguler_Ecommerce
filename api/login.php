...
if ($username !='' && $password != ''){
	...
	$query="select distrisl,bizid,xrdin,xpassword,xorg,xadd1,xmobile,(SELECT bin FROM `mlmtree` WHERE distrisl =mlminfo.distrisl and bc=1)as bin,(SELECT xbalance FROM `vospbal` WHERE xrdin = mlminfo.xrdin) as balance,membertype from mlminfo where xrdin='".$user."' and xpassword = '".$pass."'  order by bizid DESC LIMIT 1";
	$result = mysqli_query($db,$query) or die("Cannot execute query");  
	$json_response = (array());
